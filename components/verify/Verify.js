...
export const Verify = () => {
    ...
    return (
        <div >
            <h4 className='text-2xl font-semibold text-white text-center'>Forget Password</h4>
            <p className='text-lg text-white text-center'>Please input the verification code send to your email jerry73@aol.com</p>
                ...
                <div className='flex justify-center w-full gap-6'>
                    {Array(5).fill().map((_, index) => (
                        <input
                            ...
                            className={`otp-input w-15 h-15 text-center ${otp[index] ? 'border-2 border-green-600' : 'border-2 border-white'}  rounded-md  text-white focus:outline-none focus:border-green-600`}
                            maxLength="1"
                        />
                    ))}
        ...
        ${isDisabled ? 'opacity-50 cursor-not-allowed text-[#8D8D8D]' : ' text-indigo-700'}
